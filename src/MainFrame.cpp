...
MainFrame::MainFrame(wxWindow* parent, wxWindowID id, const wxString& title,
    ...
    : frameLayout(parent, id, title, pos, size, style) {
...
void MainFrame::ToggleButtons(wxObject* source) {
  std::for_each(buttons.begin(), buttons.end(),
                [source](wxToggleButton* button) {
                  if (button != source) button->SetValue(false);
                });
}

void MainFrame::OnKeyDown(wxKeyEvent& event) { canvas->ProcessKeyboard(event); }

void MainFrame::OnNewSelected(wxCommandEvent& event) {
  if (true)  // if current content has not been saved TODO: Replace true with
             // checking memento pattern
  {
    if (wxMessageBox(
            _("Unsaved changes will be lost. Do you want to continue?"),
            _("C++ Diagram editor"), wxICON_QUESTION | wxYES_NO, this) == wxNO)
      return;
  }
  canvas->clear();
}
...
{
  wxFileDialog saveFileDialog(this, wxT("Save CPPDG file"), "", "",
                              "CPPDG files (*.cppdg)|*.cppdg",
                              wxFD_SAVE | wxFD_OVERWRITE_PROMPT);
  if (saveFileDialog.ShowModal() == wxID_CANCEL) return;
    std::string path = saveFileDialog.GetPath().ToStdString();
  if (path.substr(path.size() - 6) != ".cppdg") {
    path += ".cppdg";
  }
  std::ofstream file(path);
  if (file.good()) canvas->serialize(file);
  file.close();
}

void MainFrame::OnExportSelected(wxCommandEvent& event) {
