...
void MainFrame::OnOpenSelected(wxCommandEvent &event)
{
    ...
    wxFileDialog openFileDialog(this, _("Open cppdg file"), "", "", "cppdg files (*.cppdg)|*.cppdg",
...
{
    wxFileDialog saveFileDialog(this, wxT("Save CPPDG file"), "", "", "CPPDG files (*.cppdg)|*.cppdg",
...
{
    wxFileDialog saveFileDialog(this, wxT("Save XYZ file"), "", "", "XYZ files (*.xyz)|*.xyz",
                                wxFD_SAVE | wxFD_OVERWRITE_PROMPT);
    if (saveFileDialog.ShowModal() == wxID_CANCEL)
        return;
    std::string path = saveFileDialog.GetPath().ToStdString();
