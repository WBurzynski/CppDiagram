...
ArgumentPanel::ArgumentPanel(wxWindow *parent, wxWindowID id, const wxPoint &pos, const wxSize &size, long style)
    : wxPanel(parent, id, pos, size, style), data("", "")
{
    ID = NextID++;
    ...
    ArgumentNumber =
        new wxStaticText(this, wxID_ANY, "Argument " + std::to_string(ID) + ":", wxDefaultPosition, wxDefaultSize, 0);
...
ArgumentPanel::ArgumentPanel(wxWindow *parent, dgArgument var, wxWindowID id, const wxPoint &pos, const wxSize &size,
                             long style)
    : ArgumentPanel(parent)
{
