...
void RelationPanel::setLines()
{
	...
	if (m.y < f.y)
	{
		...
		else
		{
			...
			vertical->SetSize(wxSize(2, abs(m.y - f.y) - first->GetSize().GetHeight() / 2 - second->GetSize().GetHeight() / 2 - 16));
		}
	}
	else
	{
	...
	if (m.x < s.x)
	{
		...
		else
		{
			...
			horisontal->SetSize(wxSize(abs(m.x - s.x) - second->GetSize().GetWidth() / 2 - first->GetSize().GetWidth() / 2 - 16, 2));
		}
...
RelationPanel::RelationPanel(ClassPanel* _first, ClassPanel* _second, relationType _type) : first(_first), second(_second), type(_type)
{
